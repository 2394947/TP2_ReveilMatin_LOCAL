      ...
      m_ntpClock(nullptr)
{
    ...
    this->m_ntpClock = new NTPClock(NTP_SERVER_REGION, TIMEZONE, UPDATE_INTERVAL);
    this->m_internalClock = new InternalClock(this->m_ntpClock,INTERVAL_1);
    this->m_buttonDisplayAlarms = new ButtonDisplayAvailableAlarm(pinButtonAlarms,500);
    this->m_alarmClock = new AlarmClock(this->m_display4Digits,this->m_buttonDisplayAlarms,this->m_internalClock);
